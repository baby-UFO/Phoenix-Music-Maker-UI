...
app.get('/health', (_req, res) => {
  res.json({ status: 'ok', service: 'Phoenix Music Maker API' });
});
...
app.get('/api/oembed', async (req, res) => {
  ...
  try {
    ...
    res.json({
      version: '1.0',
      type: 'rich',
      provider_name: 'Phoenix Music Maker',
...
app.get('/song/:id', async (req, res) => {
  ...
  try {
    ...
    const description = `🎵 ${song.style} • Create your own AI music free on Phoenix Music Maker`;
...
<head>
  ...
  <title>${title} | Phoenix Music Maker</title>
  ...
  <meta property="og:site_name" content="Phoenix Music Maker">
...
<body>
  <p>Redirecting to <a href="${config.frontendUrl}?song=${song.id}">Phoenix Music Maker</a>...</p>
</body>
</html>`);
  } catch (error) {
